...
// import PriorityPanel from './Component/PriorityPanel.jsx';
  import { useDispatch, useSelector } from 'react-redux';
import {addRegularTodo} from './redux-component/taskManagement.jsx';
...
export function RegularTaskItemWrapper(props) {
  const dispatch = useDispatch();
  return <RegularTaskItem {...props} dispatch={dispatch} />;
}
...
if(this.state.items.length > 0){
  ...
  completed: false,
   ...
   //用函式式 setState(prevState ⇒ newState)，React 會把當下最正確的 prevState 傳給你
   this.setState(prev => {
  const newItems = prev.items.map((item, i) =>
    i === 0
...
render(){
  const { searchResult, inputValue, toggle } = this.props;
      return (
...
constructor(props) {
  ...
  this.priorityPanelToggle = this.priorityPanelToggle.bind(this);
    ...
    regularExercisesToggle(){
      if(this.state.toggle !== false){
        this.setState({toggle : false}, ()=>{this.props.reportRegularItems(this.state.toggle);})
      }else{
        this.setState({toggle : true}, ()=>{this.props.reportRegularItems(this.state.toggle);})
      }
    }
    priorityPanelToggle(){
 ...
 />
  ...
  handleReceiveItems = (itemsFromChild) => {
        console.log('App 收到 Items:',itemsFromChild)
...
searchItem = () => {
  ...
  this.setState({
    ...
    await myApiClient.post("regular", {task: this.state.regularItems})
console.log("App saved newlist", this.state.regularItems)
      } else {
...
//       event.stopPropagation();
//       if (regularExercises) {
//         fetch(`http://localhost:8080/regular-detail`, {
//           method: 'PUT',
//           headers: {
//             "content-type": "application/json",
//             Accept: "application/json"
//           },
//           body: JSON.stringify({ content: myContent, title: newItemTitle, date: null })
//         })
...
//         //   const updatedCombinedItem = combinedItem.map((item) => {
//         //   if (item.name === newItemTitle) {
//         //     return { ...item, time: itemTime };
//         //   }
//         //   return item;
...
//         //       Accept: "application/json",
//         //     },
//         //     body: JSON.stringify({ task: { date: itemDate, taskList: updatedItem }})
//         //   })
//         //     .then(res => res.json())
//         //     .catch(error => console.error("❌ 發生錯誤：", error));
//       }
...
//   myContent,
//   setMyContent,
//   newItemTitle } = useContext(ShowContext);
//   console.log(myContent)
//   const items  = useSelector(state => state.tasks.newlistTasks);
//     const day = new Date();
//     const date = `${day.getDate()}/${day.getMonth() + 1}/${day.getFullYear()}`;
//     console.log(items)
//     useEffect(() => {
//       fetch(`http://localhost:8080/regular-reset`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
//           Accept: "application/json"
...
//     {
//       return(
// <div className="regular-items" style={{ display: regularExercises ? 'block' : 'none' }}>
      
//       {searchItem === "" ? items.map((item, index) => {
//         return (
//           <div className="regular-item" key={index}>
//             <div className="inputGroup">
//               <input
//                 type="text"
//                 className="input"
//                 onChange={(e) => setRegularNewItem(e.target.value)}
//                 placeholder="add New Task"
//               />
//               <button className="create" onClick={() => addRegularItem(regularNewItem)}>add Todo</button>
//             </div>
//             {item.task.map((task, idx) => {
//               return (
//                 <div
...
//                   }}
//                 >
//                   <button className="complete" onClick={completeFunction}>
//                     {task.completed === false ? "Done-No" : "Done-Yes"}
//                   </button>
//                   <p
//                     className="para"
...
//                     >
//                       <button
//                         onClick={(event) => { event.stopPropagation(); deleteFunction(event); }}
//                         className="delete"
//                       >
//                         Delete
//                       </button>
//                       <button
//                         onClick={(event) => { event.stopPropagation(); reNameFunction(event); }}
//                       >
//                         Rename
//                       </button>
//                       <button
...
//             <div className={showPriorityPanel ? "priority-panel show" : "priority-panel hide"}>
//               <form className='priority-filter'>
//                 <select id="sel1" value={priorityParmeter} onChange={changePriority}>
